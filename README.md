...
邀請接受後只公開會員在個人頁自行填寫的品牌聯絡人、合作 Email、LINE、Telegram、WhatsApp、Discord、Instagram 聯絡帳號與其他聯絡方式，不公開 Google 登入信箱。所有欄位皆可留白，舊版本自動帶入的 Google 信箱會在重新執行 SQL 時清除。
...
進入智慧媒合頁會先自動執行固定演算法；「重新媒合」會依最新條件更新排序與契合度。「AI 智慧排序」則使用 Gemini 產生更細緻的排序理由。真實會員不足時會補上預設候選人，因此畫面不會顯示 0 人。
